...
import { createClient, FirstDocumentMutationOptions, SanityClient } from '@sanity/client';
...
export default class SanityModuleService {
  ...
  async upsertSyncDocument<T extends SyncDocumentTypes>(type: T, data: SyncDocumentInputs<T>) {
  ...
  private transformProductForUpdate = (product: ProductDTO) => {
    return {
      set: {
        ...
        pathname: { _type: 'slug', current: '/model/' + product.handle },
        collection: {
          _type: 'reference',
          _ref: product.collection.id,
          _weak: true,
        },
      },
    };
  ...
  private transformCategoryForUpdate = (category: ProductCategoryDTO) => {
    return {
      set: {
        ...
        pathname: { _type: 'slug', current: '/category/' + category.handle },
        ...
        children_categories: category.category_children.map((child) => {
          if (!child.id) return;
          return { _type: 'reference', _ref: child.id, _weak: true };
        }),
      },
    };
  };
  ...
  private transformCollectionForCreate = (collection: ProductCollectionDTO) => {
    return {
      ...
      _id: collection.id,
      internalTitle: collection.title,
      pathname: { _type: 'slug', current: '/collection/' + collection.handle },
    };
  };
}
